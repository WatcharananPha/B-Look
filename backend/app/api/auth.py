...
def login_access_token(
    ...
    if user.role == "pending":
        raise HTTPException(
            status_code=403, detail="Account is pending approval. Please contact admin."
        )
...
def google_login(payload: GoogleLoginSchema, db: Session = Depends(get_db)) -> Any:
    try:
        ...
        if not email:
            raise HTTPException(
                status_code=400, detail="Invalid Google Token: Email missing"
            )
