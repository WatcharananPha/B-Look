...
// Helper: collapse duplicate annotations like "(บังคับไหล่สโลป+40 บาท/ตัว) (บังคับ...)" -> single occurrence
const collapseDuplicateAnnotation = (name) => {
    if (!name || typeof name !== 'string') return name;
    // exact annotation to collapse (escape for regex)
    const anno = '(บังคับไหล่สโลป+40 บาท/ตัว)';
    const esc = anno.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&');
    const re = new RegExp(`(?:${esc})(?:\\s*${esc})+`, 'g');
    let out = name.replace(re, anno);
    // collapse multiple spaces and trim
...
const getNeckExtraPrice = (neckName) => {
    ...
    // Quick override: certain special necks must add +40 (legacy forced slope pricing)
    try {
        const specialMatch = SPECIAL_NECKS_FORCE_340_UI.some(k => needle.includes(k));
        if (specialMatch) return 40;
    } catch {
        // fall through
    }
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {paginatedItems.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50 transition group border-b border-gray-200">
                                <td className="py-4 px-6 font-bold text-gray-700 text-center border-r border-gray-200">{collapseDuplicateAnnotation(item.name)}</td>
                                <td className="py-4 px-6 text-center text-gray-600 font-medium border-r border-gray-200">
                                    {(() => {
                                        // Prefer cost_price, fall back to additional_cost or price_adjustment
                                        const cp = Number(item.cost_price || 0);
                                        const add = Number(item.additional_cost || 0);
                                        const adj = Number(item.price_adjustment || 0);
                                        const display = cp || add || adj || 0;
                                        return display > 0 ? `฿${display.toLocaleString('th-TH', {minimumFractionDigits: 2, maximumFractionDigits: 2})}` : '-';
                                    })()}
                                </td>
